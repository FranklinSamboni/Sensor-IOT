...
int writeI2C(unsigned int address, unsigned int content){
	...
	buffer[0] = address;
	buffer[1] = content;
	printf("Escribiendo (RTC): !%hhX! En la direcci贸n  !%hhX! \n",content, address);
...
int setTimeRtc(char * buffer){
	...
	res = writeI2C(HOURS,nuh); // 0x02 direcci髇 del regitro de hora
	if(res != -1){
		res = writeI2C(MINUTES,num); // 0x01 direcci髇 del regitro de minutos
 		if(res != -1){
			res = writeI2C(SECONDS,nus); // 0x00 direcci髇 del regitro de segundos
 			if(res != -1){
				return 1;
			}
		}
	...
	errorRtc("Fallo Sincronizaci贸n de la hora");
	writeI2C(0x00,0x00);
	return -1;
}
...
int setDateRtc(char * buffer){
	...
	errorRtc("Fallo Sincronizaci贸n de ka fecha");
	writeI2C(0x00,0x00);
	return -1;
}
